...
@section('content')
    <div class="grid gap-8">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
                ...
                <h1 class="mt-3 text-3xl font-semibold text-slate-900 dark:text-white">Цвет профиля</h1>
                <p class="mt-2 max-w-xl text-sm text-slate-500 dark:text-slate-400">
                    Добавляйте варианты цветов профиля для подзаказов и управляйте списком.
                </p>
            </div>
            <x-button size="md" variant="solid" color="primary" data-modal-open="profile-color-create">
                Добавить цвет
            </x-button>
        </div>
        ...
        <div
            ...
            <div class="divide-y divide-slate-200/70 dark:divide-white/10">
                @forelse ($profileColors as $profileColor)
                    <div class="grid grid-cols-[1fr_auto] items-center gap-4 px-6 py-4">
                        <div>
                            <p class="text-sm font-semibold text-slate-900 dark:text-white">{{ $profileColor->name }}</p>
                        </div>
                        <div class="flex items-center gap-2">
                            <x-button size="sm" variant="faded" color="default" icon-only="true"
                                aria-label="Редактировать" data-modal-open="profile-color-edit"
                                data-modal-name="{{ $profileColor->name }}"
                                data-modal-action="{{ route('admin.profile-colors.update', $profileColor) }}">
                                <x-icon type="outline" icon="pencil" size="5"></x-icon>
                            </x-button>
                            <form method="POST" action="{{ route('admin.profile-colors.destroy', $profileColor) }}">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <div class="px-6 py-10 text-center text-sm text-slate-500 dark:text-slate-400">
                        Пока нет цветов. Добавьте первую запись.
                    </div>
                @endforelse
            </div>
        </div>
    ...
    <div class="fixed inset-0 z-40 hidden items-center justify-center bg-slate-900/50 p-4 backdrop-blur"
        data-modal="profile-color-create">
        <div
            ...
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs uppercase tracking-[0.3em] text-slate-500">Новый цвет</p>
                    <h2 class="mt-2 text-xl font-semibold text-slate-900 dark:text-white">Создание цвета профиля</h2>
                </div>
                <x-button icon-only="true" variant="ghost" color="default" data-modal-close>
                    <x-icon type="outline" icon="x" size="5"></x-icon>
                </x-button>
            </div>
            <form class="mt-6 grid gap-4" method="POST" action="{{ route('admin.profile-colors.store') }}">
                @csrf
                <x-input label="Название" name="name" required="true" placeholder="Например: Антрацит" />
    ...
    <div class="fixed inset-0 z-40 hidden items-center justify-center bg-slate-900/50 p-4 backdrop-blur"
        data-modal="profile-color-edit">
        <div
            class="w-full max-w-lg rounded-2xl border border-slate-200/70 bg-white p-6 shadow-xl dark:border-white/10 dark:bg-slate-950">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs uppercase tracking-[0.3em] text-slate-500">Редактирование</p>
                    <h2 class="mt-2 text-xl font-semibold text-slate-900 dark:text-white">Изменить цвет</h2>
                </div>
                <x-button icon-only="true" variant="ghost" color="default" data-modal-close>
                    <x-icon type="outline" icon="x" size="5"></x-icon>
                </x-button>
