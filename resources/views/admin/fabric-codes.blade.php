...
@section('content')
    <div class="grid gap-8">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
                ...
                <h1 class="mt-3 text-3xl font-semibold text-slate-900 dark:text-white">Коды ткани</h1>
                <p class="mt-2 max-w-xl text-sm text-slate-500 dark:text-slate-400">
                    Создавайте и редактируйте коды ткани для подзаказов. Выберите актуальные обозначения для работы.
                </p>
            </div>
            <x-button size="md" variant="solid" color="primary" data-modal-open="fabric-code-create">
                Добавить код
            </x-button>
        </div>
        ...
        <div
            ...
            <div class="divide-y divide-slate-200/70 dark:divide-white/10">
                @forelse ($fabricCodes as $fabricCode)
                    <div class="grid grid-cols-[1fr_auto] items-center gap-4 px-6 py-4">
                        <div>
                            <p class="text-sm font-semibold text-slate-900 dark:text-white">{{ $fabricCode->name }}</p>
                        </div>
                        <div class="flex items-center gap-2">
                            <x-button size="sm" variant="faded" color="default" icon-only="true"
                                aria-label="Редактировать" data-modal-open="fabric-code-edit"
                                data-modal-name="{{ $fabricCode->name }}"
                                data-modal-action="{{ route('admin.fabric-codes.update', $fabricCode) }}">
                                <x-icon type="outline" icon="pencil" size="5"></x-icon>
                            </x-button>
                            <form method="POST" action="{{ route('admin.fabric-codes.destroy', $fabricCode) }}">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <div class="px-6 py-10 text-center text-sm text-slate-500 dark:text-slate-400">
                        Пока нет кодов ткани. Добавьте первую запись.
                    </div>
                @endforelse
            </div>
        </div>
    ...
    <div class="fixed inset-0 z-40 hidden items-center justify-center bg-slate-900/50 p-4 backdrop-blur"
        ...
        <div
            ...
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs uppercase tracking-[0.3em] text-slate-500">Новый код</p>
                    <h2 class="mt-2 text-xl font-semibold text-slate-900 dark:text-white">Создание кода ткани</h2>
                </div>
                <x-button icon-only="true" variant="ghost" color="default" data-modal-close>
                    <x-icon type="outline" icon="x" size="5"></x-icon>
                </x-button>
            </div>
            <form class="mt-6 grid gap-4" method="POST" action="{{ route('admin.fabric-codes.store') }}">
                @csrf
                <x-input label="Название" name="name" required="true" placeholder="Например: T-120" />
    ...
    <div class="fixed inset-0 z-40 hidden items-center justify-center bg-slate-900/50 p-4 backdrop-blur"
        data-modal="fabric-code-edit">
        <div
            class="w-full max-w-lg rounded-2xl border border-slate-200/70 bg-white p-6 shadow-xl dark:border-white/10 dark:bg-slate-950">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs uppercase tracking-[0.3em] text-slate-500">Редактирование</p>
                    <h2 class="mt-2 text-xl font-semibold text-slate-900 dark:text-white">Изменить код ткани</h2>
                </div>
                <x-button icon-only="true" variant="ghost" color="default" data-modal-close>
                    <x-icon type="outline" icon="x" size="5"></x-icon>
                </x-button>
